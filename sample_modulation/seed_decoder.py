...
Purpose:
  Load experiment data (YAML + npz) saved by seed_claude.py,
  decode communication data using estimated seed values,
  and calculate the decoding BER.

Verification items:
  1. BER when decoding with the correct seed (quantum noise only)
  2. BER when decoding with the estimated seed
  3. Decoding BER for seed-match / seed-mismatch samples
"""
...
def main():
    ...
    if not os.path.exists(yaml_path):
        ...
        print("Please run seed_claude.py first to generate data.")
        return
    ...
    print("=" * 60)
    print(" Y00 Communication Decoding Verification (seed_decoder.py)")
    print("=" * 60)
...
def plot_decode_results(all_results, noise_scale):
    ...
    fig.suptitle(
        f'Y00 Communication Decoding Verification — Decoding Performance with Estimated Seed\n'
        f'(Quantum Noise Scale = {noise_scale})',
        fontsize=16, y=1.02)
    fig.tight_layout()
