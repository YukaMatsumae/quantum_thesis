...
目的:
  ...
  ※ 本スクリプトは量子ノイズなし（QUANTUM_NOISE_SCALE=0.0）のバージョン。
"""
...
def generate_seed_dataset(n_samples, lfsr_length, seq_len, noise_scale):
    """
    ランダムなシードからLFSRを駆動し、観測系列とシードビット列を生成。
...
def plot_results(results_list, lfsr_lengths):
    ...
    fig.tight_layout()
    output_filename = 'seed_prediction_results_no_noise.png'
...
def main():
    ...
    for r in results_list:
        l_len = r['lfsr_len']
        vd = r['verify_data']
        data_file = f'experiment_data_{l_len}bit_no_noise.npz'
    ...
    with open('experiment_params_no_noise.yaml', 'w', encoding='utf-8') as f:
        yaml.dump(yaml_data, f, default_flow_style=False, allow_unicode=True)
    print("  experiment_params_no_noise.yaml を保存しました。")
