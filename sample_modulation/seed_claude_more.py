...
目的:
  seed_claude.py の最適化版。LFSR長10bit超でも学習可能にするため、
  データ量・系列長・モデル容量を大幅に強化。
...
# ── モデル保存ディレクトリ ──
MODEL_SAVE_DIR = 'saved_models_more'
...
CONFIG = {
    ...
    12: (256,  256,  8, 3,   300_000,  30_000,   512, 400, 256, 2),
    14: (512,  512, 16, 3,   800_000, 100_000,  1024, 500, 128, 4),
    16: (1024, 1024, 16, 4, 2_000_000, 200_000,  2048, 600,  64, 8),
}
...
def plot_results(results_list, lfsr_lengths):
    ...
    fig.tight_layout()
    output_filename = 'seed_prediction_results_more.png'
...
def main():
    ...
    print(f"※ 強化版: 大規模データ・拡張モデル・長系列対応\n")
    ...
    for r in results_list:
        l_len = r['lfsr_len']
        vd = r['verify_data']
        data_file = f'experiment_data_{l_len}bit_more.npz'
    ...
    with open('experiment_params_more.yaml', 'w', encoding='utf-8') as f:
        yaml.dump(yaml_data, f, default_flow_style=False, allow_unicode=True)
    print("  experiment_params_more.yaml を保存しました。")
...
if __name__ == "__main__":
    ...
    log_filename = f'seed_claude_more_log_{timestamp}.txt'
